 ...
 * Fields: full_name, email, address, school
...
try {
    ...
    $stmt = $pdo->prepare(
        'INSERT INTO applications (ref_code, full_name, email, address, school, status)
         VALUES (?, ?, ?, ?, ?, "submitted")'
    );
    $stmt->execute([$refCode, $name, $email, $address, $school]);
